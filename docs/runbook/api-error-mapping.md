...
> **STATUS: ACTIVE, last verified 2026-05-28.** Introduced in iter 2 of the Vehicles slice (`feat/vehicles-write-path`). This procedure documents the project-wide convention for translating database-layer and validation-layer failures into HTTP status codes. Other modules adopting the write-path pattern follow this mapping rather than inventing their own. As of iter 7 of the Drivers slice, both Vehicles and Drivers write paths use this mapping for `P2002` (duplicate `registrationNumber` / `licenseNumber`) and `P2025` (delete or update of a missing row). Iter 20 (Fuel logs write path) extends the table with the `P2003`-on-insert / on-update branch — a stale `vehicleId` or `tripId` on a POST/PATCH surfaces as HTTP 400 with the offending field token in the message, so the matching create / edit forms can highlight the right picker.
...
| `P2003` (delete-when-referenced) | Foreign-key constraint failed because another row references the target | 409 Conflict | `ConflictException` | Used for `DELETE` of a parent row whose child rows would orphan. Message names the dependent resource (e.g., `Cannot delete customer: it is referenced by other records.` — `CustomersService.delete`; `Cannot delete vehicle: it is referenced by other records.` — `VehiclesService.delete`). The web client surfaces a "View related records" link in the delete dialog deep-linking to the referencing list (e.g., `/jobs?customerId=…`). |
| `P2003` (insert / update with stale FK) | Foreign-key constraint failed because the referenced row does not exist | 400 Bad Request | `BadRequestException` | Used by `POST` / `PATCH` when the client sent an id that points at a row that no longer exists (e.g., `vehicleId` deleted between the form load and submit). Message names the offending field (e.g., `Vehicle <id> does not exist.`, `Trip <id> does not exist.` — `FuelLogsService.create` / `FuelLogsService.update` via `mapFuelLogWriteError`; `Customer <id> does not exist.` — `JobsService.create`). The web `actions.ts` layer pattern-matches the message to set the right field token (`vehicleId` / `tripId` / `customerId`) so the form highlights the right picker. Distinguished from the delete-when-referenced branch by which side of the FK violation we are on: insert/update never has a dependent resource to name. |
