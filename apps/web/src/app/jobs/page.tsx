...
// omits skip at page 0 so the canonical /jobs URL stays clean.
...
// Sortable-header link — toggles dir on the active column, else sets the
// new column desc. Same convention as Customers / Trips / Drivers.
function sortParams(
...
function SortableHeader({
  ...
  const href = `/jobs${sortParams(searchParams, column, activeColumn, activeDir)}`;
...
function Pagination({ total, skip, take, searchParams }: PaginationProps): React.ReactElement {
  ...
  const prevHref = `/jobs${paginationParams(searchParams, Math.max(0, skip - safeTake))}`;
  const nextHref = `/jobs${paginationParams(searchParams, skip + safeTake)}`;
        ...
        {pages.map((p, idx) =>
          p === "ellipsis" ? (
            ...
            <Button key={p} asChild variant="ghost" size="sm" className="tabular-nums">
              <Link href={`/jobs${paginationParams(searchParams, (p - 1) * safeTake)}`}>{p}</Link>
            </Button>
          ),
        )}
        {nextDisabled ? (
                  ...
                  {data.items.map((j) => (
                    ...
                    <TableRow key={j.id} className="relative cursor-pointer">
                      ...
                      <TableCell className="text-text-secondary text-right tabular-nums">
                        {formatDate(j.scheduledStartDate)}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
